<!DOCTYPE html>
<html lang="de">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Mathe-Lern-App</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="manifest" href=""> <!-- Will be set dynamically -->
    <meta name="theme-color" content="#8B5CF6">
    <link rel="apple-touch-icon" href="https://placehold.co/192x192/8B5CF6/FFFFFF?text=Math">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;700&display=swap" rel="stylesheet">
    <style>
        body {
            font-family: 'Inter', sans-serif;
        }
        .challenge-card {
            transition: transform 0.2s ease-in-out, box-shadow 0.2s ease-in-out;
        }
        .challenge-card:hover {
            transform: scale(1.05);
            box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05);
        }
        .btn {
            transition: background-color 0.2s ease-in-out;
        }
    </style>
</head>
<body class="bg-gray-100 text-gray-800">

    <div id="app" class="container mx-auto p-4 max-w-lg">

        <!-- Initial Screen: Challenge Selection -->
        <div id="challenge-selection" class="text-center">
            <h1 class="text-4xl font-bold text-purple-600 mb-4">Mathe-Challenge</h1>
            <p class="text-lg mb-8">Wähle deine Herausforderung!</p>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div onclick="startChallenge(7)" class="challenge-card bg-white p-6 rounded-lg shadow-md cursor-pointer">
                    <h2 class="text-2xl font-bold mb-2">7 Tage</h2>
                    <p>Eine Woche voller Mathe-Spaß!</p>
                </div>
                <div onclick="startChallenge(14)" class="challenge-card bg-white p-6 rounded-lg shadow-md cursor-pointer">
                    <h2 class="text-2xl font-bold mb-2">14 Tage</h2>
                    <p>Zwei Wochen, um deine Fähigkeiten zu testen.</p>
                </div>
                <div onclick="startChallenge(30)" class="challenge-card bg-white p-6 rounded-lg shadow-md cursor-pointer">
                    <h2 class="text-2xl font-bold mb-2">30 Tage</h2>
                    <p>Ein ganzer Monat voller Herausforderungen!</p>
                </div>
            </div>
        </div>

        <!-- Exercise Screen -->
        <div id="exercise-screen" class="hidden">
            <div class="flex justify-between items-center mb-4">
                <h1 class="text-2xl font-bold text-purple-600">Tägliche Aufgabe</h1>
                <div id="stats" class="text-right">
                    <p>Tag: <span id="current-day"></span>/<span id="total-days"></span></p>
                    <p>Punkte: <span id="current-score"></span></p>
                </div>
            </div>
            <div class="bg-white p-8 rounded-lg shadow-md">
                <p class="text-gray-600 mb-2" id="problem-topic"></p>
                <p class="text-2xl mb-6" id="problem-text"></p>
                <input type="text" id="answer-input" class="w-full p-3 border border-gray-300 rounded-lg mb-4 focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder="Deine Antwort...">
                <button id="submit-btn" onclick="submitAnswer()" class="btn w-full bg-purple-600 text-white p-3 rounded-lg hover:bg-purple-700">Antwort abgeben</button>
                <button id="solution-btn" onclick="showSolution()" class="btn w-full bg-gray-400 text-white p-3 rounded-lg hover:bg-gray-500 mt-2">Lösung anzeigen</button>
            </div>
             <div id="feedback" class="mt-4 p-4 rounded-lg hidden"></div>
        </div>

        <!-- End Screen -->
        <div id="end-screen" class="hidden text-center">
            <h1 class="text-4xl font-bold text-purple-600 mb-4">Challenge beendet!</h1>
            <div id="medal" class="text-6xl mb-4"></div>
            <p class="text-lg mb-2">Deine Punktzahl: <span id="final-score"></span></p>
            <p class="text-lg mb-8">Du hast <span id="medal-name"></span> erreicht!</p>
            <button onclick="restartApp()" class="btn bg-purple-600 text-white p-3 rounded-lg hover:bg-purple-700">Neue Challenge starten</button>
        </div>
    </div>

    <script>
        // Expanded list of problems from the user's document
        const problems = [
            { topic: "Rationale Zahlen – Addition & Subtraktion", question: "Berechne: (-7) + 12 = ?", solution: "5" },
            { topic: "Rationale Zahlen – Addition & Subtraktion", question: "Berechne: 15 - (-4) = ?", solution: "19" },
            { topic: "Rationale Zahlen – Addition & Subtraktion", question: "Berechne: (-9) + (-6) = ?", solution: "-15" },
            { topic: "Terme – Vereinfachen & Werte berechnen", question: "Vereinfache den Term: 3x + 5y - x + 2y", solution: "2x + 7y" },
            { topic: "Terme – Vereinfachen & Werte berechnen", question: "Berechne den Wert des Terms 2a - 3b für a = 4 und b = -2.", solution: "14" },
            { topic: "Prozentrechnung – Grundaufgaben", question: "Wie viel sind 15% von 200 €?", solution: "30 €" },
            { topic: "Lineare Gleichungen – Lösen", question: "Löse nach x auf: 3x + 7 = 22", solution: "5" },
            { topic: "Lineare Funktionen – Grundlagen", question: "Ist der Punkt P(3|5) ein Punkt der Funktion y = 2x - 1? (Ja/Nein)", solution: "Ja" },
            { topic: "Zinsrechnung", question: "Wie viel Zinsen erhält man für 1500 € bei einem Zinssatz von 2% pro Jahr nach einem Jahr?", solution: "30 €" },
            { topic: "Lineare Gleichungssysteme", question: "Löse das LGS: I) y = 2x + 1; II) y = -x + 4", solution: "x=1, y=3" },
            { topic: "Geometrie – Flächeninhalt", question: "Ein Rechteck hat die Seitenlängen 8 cm und 5 cm. Berechne den Flächeninhalt.", solution: "40 cm²" },
            { topic: "Daten und Boxplots", question: "Gegeben sind die Daten: 5, 8, 3, 10, 7, 2, 6. Bestimme den Median.", solution: "6" },
            { topic: "Rationale Zahlen – Multiplikation & Division", question: "Berechne: (-5) * 8 = ?", solution: "-40" },
            { topic: "Lineare Funktionen – Gleichung aufstellen", question: "Eine Gerade hat die Steigung m = 2 und geht durch den Punkt P(1|5). Bestimme die Funktionsgleichung.", solution: "y = 2x + 3" },
            { topic: "Terme – Klammern auflösen", question: "Vereinfache: (x + 2)(x - 3)", solution: "x² - x - 6" },
            { topic: "Geometrie – Winkelsumme", question: "Ein Dreieck hat die Winkel 45° und 60°. Wie groß ist der dritte Winkel?", solution: "75°" },
            { topic: "Proportionale Zuordnungen", question: "3 Äpfel kosten 1,50 €. Wie viel kosten 5 Äpfel?", solution: "2.50 €" },
            { topic: "Vermischte Aufgaben", question: "Ein Fahrrad kostet 250 €. Es wird um 10% reduziert. Wie viel kostet es jetzt?", solution: "225 €" },
            { topic: "Lineare Funktionen – Steigung", question: "Gegeben ist die Funktion y = -3x + 4. Bestimme die Steigung.", solution: "-3" },
            { topic: "Terme – Potenzen", question: "Vereinfache: (4x)² = ?", solution: "16x²" }
        ];

        // Application state
        let appState = {
            challengeDuration: 0,
            currentDay: 0,
            score: 0,
            usedProblems: [],
            currentProblem: null,
            lastExerciseDate: null,
        };

        // DOM Elements
        const challengeSelectionEl = document.getElementById('challenge-selection');
        const exerciseScreenEl = document.getElementById('exercise-screen');
        const endScreenEl = document.getElementById('end-screen');
        const feedbackEl = document.getElementById('feedback');
        const answerInputEl = document.getElementById('answer-input');
        const submitBtn = document.getElementById('submit-btn');
        const solutionBtn = document.getElementById('solution-btn');

        // --- State Management ---
        function saveState() {
            localStorage.setItem('mathAppState', JSON.stringify(appState));
        }

        function loadState() {
            const savedState = localStorage.getItem('mathAppState');
            if (savedState) {
                appState = JSON.parse(savedState);
            }
        }

        function isNewDay() {
            const today = new Date().toDateString();
            return appState.lastExerciseDate !== today;
        }

        // --- App Flow ---
        function startChallenge(days) {
            appState.challengeDuration = days;
            appState.currentDay = 1;
            appState.score = 0;
            appState.usedProblems = [];
            appState.lastExerciseDate = null;
            saveState();
            showExerciseScreen();
        }

        function showExerciseScreen() {
            if (appState.challengeDuration > 0 && appState.currentDay > appState.challengeDuration) {
                showEndScreen();
                return;
            }

            challengeSelectionEl.classList.add('hidden');
            endScreenEl.classList.add('hidden');
            exerciseScreenEl.classList.remove('hidden');
            
            document.getElementById('current-day').innerText = appState.currentDay;
            document.getElementById('total-days').innerText = appState.challengeDuration;
            document.getElementById('current-score').innerText = appState.score;

            if (isNewDay()) {
                let availableProblems = problems.filter((p, index) => !appState.usedProblems.includes(index));
                if (availableProblems.length === 0) {
                    // Reset if all problems have been used
                    appState.usedProblems = [];
                    availableProblems = problems;
                }
                const randomIndex = Math.floor(Math.random() * availableProblems.length);
                const problemIndex = problems.indexOf(availableProblems[randomIndex]);
                
                appState.currentProblem = { ...problems[problemIndex], index: problemIndex };
                appState.usedProblems.push(problemIndex);
                
                document.getElementById('problem-topic').innerText = appState.currentProblem.topic;
                document.getElementById('problem-text').innerText = appState.currentProblem.question;
                
                answerInputEl.value = '';
                answerInputEl.disabled = false;
                submitBtn.disabled = false;
                solutionBtn.disabled = false;
                feedbackEl.classList.add('hidden');
            } else {
                 document.getElementById('problem-topic').innerText = "Bereits erledigt";
                 document.getElementById('problem-text').innerText = "Du hast deine heutige Aufgabe bereits erledigt. Komm morgen wieder!";
                 answerInputEl.disabled = true;
                 submitBtn.disabled = true;
                 solutionBtn.disabled = true;
            }
        }

        function submitAnswer() {
            const userAnswer = answerInputEl.value.trim();
            
            if (!userAnswer) {
                showFeedback("Bitte gib eine Antwort ein.", 'yellow');
                return;
            }

            const isCorrect = userAnswer.toLowerCase().replace(/ /g, '') === appState.currentProblem.solution.toLowerCase().replace(/ /g, '');

            if (isCorrect) {
                appState.score += 6;
                showFeedback("Richtig! Super gemacht!", 'green');
            } else {
                appState.score += 4;
                showFeedback("Nicht ganz richtig, aber gut versucht!", 'red');
            }

            appState.lastExerciseDate = new Date().toDateString();
            appState.currentDay += 1;
            saveState();

            answerInputEl.disabled = true;
            submitBtn.disabled = true;
            solutionBtn.disabled = true;

            setTimeout(() => {
                showExerciseScreen();
            }, 2000);
        }
        
        function showSolution() {
            showFeedback(`Die richtige Antwort ist: ${appState.currentProblem.solution}`, 'blue');
            submitBtn.disabled = true; // Disable submit after seeing solution
            solutionBtn.disabled = true;
        }

        function showEndScreen() {
            exerciseScreenEl.classList.add('hidden');
            endScreenEl.classList.remove('hidden');

            const maxPoints = (appState.challengeDuration) * 6;
            const percentage = maxPoints > 0 ? (appState.score / maxPoints) * 100 : 0;
            let medal = '🥉';
            let medalName = 'Bronze';

            if (percentage >= 80) {
                medal = '🥇';
                medalName = 'Gold';
            } else if (percentage >= 65) {
                medal = '🥈';
                medalName = 'Silber';
            }

            document.getElementById('medal').innerText = medal;
            document.getElementById('final-score').innerText = appState.score;
            document.getElementById('medal-name').innerText = medalName;
        }

        function restartApp() {
            localStorage.removeItem('mathAppState');
            // Reset state object to its initial values
            appState = {
                challengeDuration: 0,
                currentDay: 0,
                score: 0,
                usedProblems: [],
                currentProblem: null,
                lastExerciseDate: null,
            };
            endScreenEl.classList.add('hidden');
            challengeSelectionEl.classList.remove('hidden');
        }

        function showFeedback(message, color) {
            const colorClasses = {
                'green': 'bg-green-200 text-green-800',
                'red': 'bg-red-200 text-red-800',
                'blue': 'bg-blue-200 text-blue-800',
                'yellow': 'bg-yellow-200 text-yellow-800',
            };
            feedbackEl.innerText = message;
            feedbackEl.className = `mt-4 p-4 rounded-lg ${colorClasses[color] || ''}`;
            feedbackEl.classList.remove('hidden');
        }

        // --- PWA Setup ---
        function setupPWA() {
            const manifest = {
                "name": "Mathe-Lern-App",
                "short_name": "Mathe-App",
                "start_url": ".",
                "display": "standalone",
                "background_color": "#ffffff",
                "theme_color": "#8B5CF6",
                "description": "Eine App zum Üben von Matheaufgaben.",
                "icons": [{
                    "src": "https://placehold.co/192x192/8B5CF6/FFFFFF?text=Math",
                    "sizes": "192x192",
                    "type": "image/png"
                }, {
                    "src": "https://placehold.co/512x512/8B5CF6/FFFFFF?text=Math",
                    "sizes": "512x512",
                    "type": "image/png"
                }]
            };
            const manifestBlob = new Blob([JSON.stringify(manifest)], {type: 'application/json'});
            const manifestURL = URL.createObjectURL(manifestBlob);
            document.querySelector('link[rel="manifest"]').href = manifestURL;

            if ('serviceWorker' in navigator) {
                const swCode = `
                    self.addEventListener('install', event => {
                        console.log('Service worker installing...');
                    });
                    self.addEventListener('fetch', event => {
                        // Basic cache-first strategy can be added here later
                        console.log('Fetching:', event.request.url);
                    });
                `;
                const swBlob = new Blob([swCode], {type: 'application/javascript'});
                const swURL = URL.createObjectURL(swBlob);
                
                navigator.serviceWorker.register(swURL)
                    .then(registration => {
                        console.log('ServiceWorker registration successful with scope: ', registration.scope);
                    })
                    .catch(err => {
                        console.log('ServiceWorker registration failed: ', err);
                    });
            }
        }

        // --- Initial Load ---
        window.onload = () => {
            loadState();
            if (appState.challengeDuration > 0) {
                showExerciseScreen();
            }
            setupPWA();
        };
    </script>
</body>
</html>
